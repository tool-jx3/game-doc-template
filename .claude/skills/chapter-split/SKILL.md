...
Navigation behavior:
- `generate_nav.py` now flattens single-file sections into direct sidebar links.
- Multi-file sections still use Starlight autogenerate groups.
- `starlight-auto-sidebar` is optional and only needed when nested autogenerated groups need `_meta.yml` customization such as label overrides, collapse state, or depth rules.
